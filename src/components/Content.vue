<template>
  <Filters
    @update:milk="handleMilkOptionsUpdate"
    ...
    <div class="bg-polka-dots">
      ...
      <div v-if="noItems" class="p-8">
        <div class="flex flex-col items-center justify-center mt-8">
          <h1 class="text-4xl font-bold text-gray-800 mb-4">
            No results matched your search for "{{ searchQuery }}"
          </h1>
          <p class="text-xl text-gray-600 mb-2">
            We apologize for the inconvenience. Please try again with different
            keywords.
          </p>
          <p
            v-if="searchQuery.split(' ').length > 1"
            class="text-xl text-gray-600"
          >
            Try your search with fewer words to get better results.
          </p>
        </div>
      </div>
          ...
          >
            <div class="md:flex md:items-center md:justify-between">
              <h2
                ...
                class="text-4xl font-bold tracking-tight text-gray-700"
              >
      ...
      <div v-else>
        <div v-for="category in filteredCategories" class="bg-inherit">
          <section
            v-if="getFilteredAndSortedItems(category.name).length > 0"
            aria-labelledby="items-listed"
            :id="`${category.name}`"
          >
            <div
              class="mx-auto max-w-screen-sm md:max-w-screen-md lg:max-w-screen-lg xl:max-w-screen-xl 2xl:max-w-screen-2xl px-4 py-6 sm:px-6 sm:mt-12 lg:px-8 lg:mt-12 bg-inherit -pt-24"
            >
              ...
              <ItemCards
                :items="getFilteredAndSortedItems(category.name)"
                :itemCurrency="itemCurrency"
                @openModal="openModal"
              />
            </div>
            <div
              v-if="
                getFilteredAndSortedItems(category.name, Infinity).length > 6
              "
              class="mt-4"
            >
              <RouterLink
...
const handleScroll = () => {
  if (heroImageContainer.value) {
    ...
    heroImageContainer.value.style.transform = `translateY(${
      scrollPosition * 0.25
    }px)`;
  }
};
...
.store-bg {
  ...
  opacity: 0.8;
  background-image: radial-gradient(#fde68a 0.5px, transparent 0.5px),
    radial-gradient(#fde68a 0.5px, #f9fafb 0.5px);
